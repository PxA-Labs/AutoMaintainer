...
interface MonacoInstance {
  ...
  KeyCode: { KeyK: number; KeyS: number; KeyW: number };
...
export default function WebIDE({ repoUrl, branchName }: WebIDEProps) {
  ...
  const handleEditorMount: OnMount = (editor, monaco) => {
    ...
    // Register Cmd+S / Ctrl+S keybinding in Monaco
    if (monaco.KeyCode.KeyS) {
      editor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.KeyS, () => {
        handleSaveRef.current();
      });
    }

    // Register Cmd+W / Ctrl+W keybinding in Monaco
    if (monaco.KeyCode.KeyW) {
  ...
  const fetchTree = useCallback(async () => {
    // Repo and branch can both change while a request is in flight, so only the
  ...
  // Load the tree for the active repo and branch. Keyed on fetchTree, which
  // changes whenever repoUrl or branchName does, so the IDE follows the branch
  // the agent is working on without discarding open tabs on a branch switch.
  useEffect(() => {
    setLoadingTree(true);
    fetchTree();
  }, [fetchTree]);
  ...
  }, [activeTab, editedContents, fileContents, repoUrl, branchName]);
  ...
  const closeTabByPathRef = useRef(closeTabByPath);
  useEffect(() => {
    closeTabByPathRef.current = closeTabByPath;
  }, [closeTabByPath]);
        ...
        <div className="h-9 flex items-center px-4 text-xs font-semibold text-[#cccccc] uppercase tracking-wider shrink-0 flex-row justify-between">
          ...
          {activeSidebarMode === "explorer" && (
            <div className="flex gap-2">
              ...
              >
                <FilePlus className="w-4 h-4 text-zinc-400 hover:text-white" />
              </button>
              <button
              ...
              >
                <FolderPlus className="w-4 h-4 text-zinc-400 hover:text-white" />
              </button>
            </div>
          )}
        </div>
