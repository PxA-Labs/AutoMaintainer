...
def create_presentation():
    ...
    def create_content_slide(title_text):
        ...
        line = slide.shapes.add_shape(
            MSO_SHAPE.RECTANGLE, Inches(0.5), Inches(1.2), Inches(12.333), Inches(0.05)
        )
        line.fill.solid()
    ...
    add_bullet(
        tf, "Uses GitNexus MCP for zero-server semantic codebase ingestion.", level=1
    )
    add_bullet(
        tf,
        "Architect Agent deeply analyzes file trees and architectures before any action.",
        level=1,
    )
    ...
    add_bullet(
        tf,
        "Reviewer Agent acts as a PM, independently evaluating feature proposals against strict directives.",
        level=1,
    )
    add_bullet(
        tf,
        "Maintainer Agent acts as QA, choosing to approve (LGTM) or reject and loop Pull Requests based on bugs.",
        level=1,
    )
    ...
    add_bullet(
        tf,
        "Operates natively via PyGithub to open Issues, write code, branch, commit, and merge—100% hands-free.",
        level=1,
    )
    ...
    add_bullet(
        ltf, "Require constant human prompting, steering, and hand-holding.", level=1
    )
    add_bullet(
        ltf,
        "Developers still spend >50% of time fixing bugs and managing technical debt.",
        level=1,
    )
    ...
    right_box = slide.shapes.add_textbox(
        Inches(6.8), Inches(1.5), Inches(5.8), Inches(5)
    )
    ...
    add_bullet(
        rtf,
        "Self-correcting iteration loops catch and fix bugs before human review.",
        level=1,
    )
    ...
    add_bullet(
        tf,
        "Frontend: Next.js (React), Tailwind CSS, Framer Motion, WebSockets",
        level=1,
    )
    ...
    add_bullet(
        ftf,
        "1. Trigger -> 2. Ingest Architecture -> 3. Brainstorm Feature -> 4. Open GitHub Issue",
        level=1,
        size=Pt(18),
    )
    add_bullet(
        ftf,
        "5. Review Issue -> 6. Write Code -> 7. Open Pull Request -> 8. Code Review",
        level=1,
        size=Pt(18),
    )
    add_bullet(
        ftf, "9. Merge if LGTM, OR Fix & Re-Push if Bug Detected.", level=1, size=Pt(18)
    )
    ...
    table_shape = slide.shapes.add_table(
        6, 2, Inches(0.5), Inches(1.5), Inches(12), Inches(5)
    )
    ...
    roles = [
        (
            ...
            "Scans the target repository's file tree and README. Assesses the tech stack and project state, then generates a strict architectural directive.",
        ),
        (
            "2. Visionary (PM)",
            "Reads the directive and ideates an innovative feature. It opens a native GitHub Issue detailing the proposed feature.",
        ),
        (
            "3. Reviewer (Gatekeeper)",
            "Evaluates the feature against the original directive. If approved, it comments on the Issue. If rejected, it closes it.",
        ),
        (
            "4. Implementer (Dev)",
            "Writes the actual code to build the feature. Pushes to a new branch and opens a Pull Request. Reads feedback from loops to fix bugs.",
        ),
        (
            "5. Maintainer (QA)",
            "Reviews the PR code. Spots flaws, leaves comments, and routes pipeline back to the Implementer. Merges if 'LGTM'.",
        ),
    ]
    ...
    add_bullet(
        tf,
        "Unlike basic generators, AutoMaintainer evaluates its own code output.",
        level=0,
        bold=True,
        size=Pt(26),
    )
    ...
    add_bullet(
        tf,
        "If a bug, syntax error, or logic flaw is found, it leaves a GitHub PR comment.",
        level=1,
    )

    add_bullet(
        tf, "The Iteration Cycle:", level=0, bold=True, size=Pt(24), color=accent_color
    )
    add_bullet(
        tf,
        "Pipeline routes backwards. The Implementer reads the feedback and pushes a new commit to the branch.",
        level=1,
    )
    add_bullet(
        tf,
        "Bounded to a maximum of 3 iteration cycles to prevent infinite loops.",
        level=1,
    )

    add_bullet(
        tf,
        "Result: >85% of logic errors are self-corrected before human intervention is ever needed.",
        level=0,
        bold=True,
        size=Pt(22),
        color=success_color,
    ...
    add_bullet(
        tf,
        "Powered by Groq's LPU inference, the entire cycle (Architecture -> PR -> Review -> Merge) completes in < 20 seconds.",
        level=1,
    )
    add_bullet(
        tf,
        "Human average for equivalent context-loading + coding + PR review: ~45 minutes.",
        level=1,
    )

    add_bullet(
        tf, "2. Local Zero-Server Code Intelligence", level=0, bold=True, size=Pt(24)
    )
    add_bullet(
        tf,
        "GitNexus MCP processes 10,000+ lines of codebase locally in < 2 seconds.",
        level=1,
    )
    add_bullet(
        tf,
        "Ensures total privacy. Code is semantically mapped without being sent to external databases.",
        level=1,
    )
    ...
    add_bullet(
        tf,
        "Deploys anywhere instantly via Docker, with native Hugging Face Spaces compatibility.",
        level=1,
    )
    ...
    table_shape = slide.shapes.add_table(
        5, 2, Inches(0.5), Inches(1.5), Inches(12), Inches(5)
    )
    table = table_shape.table
    ...
    roadmap = [
        (
            "Phase 1: SaaS",
            "Dual-Storage Adapter using Supabase PostgreSQL for 100k+ concurrent user scale.",
        ),
        (
            "Phase 2: CLI",
            "Native CLI binary bypassing GitHub API to work directly on uncommitted local git trees.",
        ),
        (
            "Phase 3: SDKs",
            "Python & Node.js SDKs to embed LangGraph autonomy into GitHub Actions & GitLab CI.",
        ),
        (
            "Phase 4: MCP",
            "Exposing the crew as a Universal Model Context Protocol (MCP) Server. Cursor & Devin can 'hire' us.",
        ),
        (
            "Phase 5: IDE",
            "Native VS Code Extension. Agents fix bugs silently in the background as the developer types.",
        ),
    ]
    ...
    tf = txBox.text_frame
    tf.text = (
        "AutoMaintainer isn't just an assistant; it is a fully autonomous AI Colleague."
    )
